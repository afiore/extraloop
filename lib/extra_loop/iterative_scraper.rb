...
class IterativeScraper < ScraperBase
  ...
  def set_iteration(param, collection)
    @iteration_set = Array(collection).collect &:to_s
    set_iteration_param(param)
    self
  end

  def set_iteration_proc(param, selector, closure)
    set_iteration_param(param)
    @iteration_extractor = Extractor.new(:pagination, selector, closure)
    self
  end
  ...
  def extract_iteration_set(response)
    @iteration_extractor.extract_list(response.body).collect(&:to_s)
  end
end
